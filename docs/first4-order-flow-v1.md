...
**Subsequent authorized deployment and enablement:** release `a7ac6db9c81c` is active
with slot 1 observation enabled in `TBT_TRADES_TBT_QUOTES` mode. The shared-stream
budget correction counts each canonical Last once. No data subscription was purchased.
See [current deployment](CURRENT-DEPLOYMENT.md) for exact configuration and evidence.
The original implementation-only boundary below describes the preceding offline phase.
