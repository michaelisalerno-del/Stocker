...
async function runDetail(runId) {
  ...
  const details = ["market", "environment", "account", "currency", "market_state", "session", "screen_state", "screen_timestamp", "watchlist_size", "last_checkpoint", "next_checkpoint", "risk_per_trade", "max_concurrent_positions"];
  const activity = run.activity_screen;
  const activityColumns = [{ key: "symbol", label: "Stock" }, { key: "currency", label: "Currency" }, { key: "scan_hit_count", label: "Scanner hits", numeric: true }, { key: "aggregate_screen_score", label: "Activity score", numeric: true }, { key: "best_component_rank", label: "Best rank", numeric: true }, { label: "Selection", render: (row) => row.selected ? "Watchlist" : "Outside capacity limit" }];
  const activityPanel = activity ? `<section class="section"><div class="section-head"><h2>Activity filter</h2></div><p>${number(activity.candidates.length)} activity candidates · ${number(activity.candidates.filter(row => row.selected).length)} selected for contract and history checks. ${esc(activity.reason)}</p><p class="muted">Ranked by scanner agreement, activity score and best rank. The watchlist is limited by tick-feed capacity. Concurrent markets share the same feed budget. This PAPER filter has not been validated for profitability.</p><details><summary>Inspect stock selection</summary>${table(activityColumns, activity.candidates)}</details></section>` : "";
  ...
  main.innerHTML = `${head(run.display_name, `${run.market || run.universe} / ${run.environment}`)}
    ...
    ${activityPanel}<section class="section"><div class="section-head"><h2>Performance</h2><div class="tabs">${periods}</div></div><div class="metric-strip performance-strip">${metrics}</div>${table(historyColumns, performance.history)}</section>`;
