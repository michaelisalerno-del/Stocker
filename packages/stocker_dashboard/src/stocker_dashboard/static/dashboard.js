...
async function runDetail(runId) {
  ...
  main.innerHTML = `${head(run.display_name, `${run.market || run.universe} / ${run.environment}`)}
    ...
    ${discoveryPanel}${activityPanel}<section class="section"><div class="section-head"><h2>Performance</h2><div class="tabs">${periods}</div></div><div class="metric-strip performance-strip">${metrics}</div>${table(historyColumns, performance.history)}</section>`;
