...
CREATE TABLE IF NOT EXISTS quiet_option_risk_observation_v0 (
    ...
    strategy_type TEXT NOT NULL CHECK (
        strategy_type IN (
            'UNDERLYING_LONG', 'LONG_OPTION', 'SHORT_PUT',
            'BULL_PUT_SPREAD', 'DEFINED_RISK_OPTION'
        )
    ),
    dte_bucket TEXT NOT NULL CHECK (dte_bucket IN ('0DTE', '1DTE', '3_TO_5_DTE')),
...
CREATE INDEX IF NOT EXISTS idx_quiet_option_risk_observation_parent
    ON quiet_option_risk_observation_v0(
        observation_id, dte_bucket, horizon_label, observed_at_utc
    );
