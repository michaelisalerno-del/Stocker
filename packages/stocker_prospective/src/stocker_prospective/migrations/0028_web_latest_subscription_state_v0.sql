...
INSERT INTO web_latest_subscription_state_v0(
    run_id, subscription_key, event_id, subscription_kind, status
)
SELECT event.run_id, event.subscription_key, event.id, event.subscription_kind, event.status
FROM subscription_lifecycle_event_v0 AS event
JOIN (
    SELECT run_id, subscription_key, MAX(id) AS event_id
...
BEGIN
    INSERT INTO web_latest_subscription_state_v0(
        ...
        NEW.run_id, NEW.subscription_key, NEW.id, NEW.subscription_kind, NEW.status
    )
    ON CONFLICT(run_id, subscription_key) DO UPDATE SET
        event_id = excluded.event_id,
        subscription_kind = excluded.subscription_kind,
