...
function renderHealth() {
  ...
  [
    ...
    metric("Frozen feature runtime", health.active_bundle.feature_runtime?.installed ? "INSTALLED + VERIFIED" : "NOT INSTALLED", health.active_bundle.feature_runtime?.contract_version || "Artifact transform runtime unavailable", health.active_bundle.feature_runtime?.installed ? "ok" : "danger"),
    metric("Parallel source evidence", health.parallel_validation?.enabled ? (health.parallel_validation?.credential_configured ? "CONFIGURED" : "CREDENTIAL MISSING") : "DISABLED", clean(health.parallel_validation?.latest_capture?.status || health.parallel_validation?.blocker || "No capture yet"), health.parallel_validation?.enabled && health.parallel_validation?.credential_configured ? "ok" : "danger"),
...
function renderSafety() {
  ...
  const cards = [
    ...
    safetyCard("Frozen feature runtime", bundle.feature_runtime?.installed ? "VERIFIED" : "ABSENT", `${clean(bundle.feature_runtime?.contract_version)} // registry alone never authorizes scoring.`, bundle.feature_runtime?.installed ? "ok" : "danger"),
    ...
    safetyCard("Parallel source evidence", health.parallel_validation?.enabled ? "ENABLED" : "DISABLED", `${clean(health.parallel_validation?.provider)} // credential configured ${clean(health.parallel_validation?.credential_configured)} // latest ${clean(health.parallel_validation?.latest_capture?.status)} // scoring allowed FALSE.`, health.parallel_validation?.enabled && health.parallel_validation?.credential_configured ? "ok" : "danger"),
