...
from stocker_research.legacy_prospective.signed_market_shock_v1 import MarketShockBarV1
    ...
    def _valid_thresholds(self) -> OpeningTransitionThresholdsV1:
        ...
        if self.calibration_complete_v1:
            if any(value is None or not math.isfinite(value) for value in values):
                raise ValueError("complete opening calibration requires finite thresholds")
            ...
            if (
                self.market_opening_return_q10_v1 > self.market_opening_return_q90_v1
                or self.market_opening_range_q75_v1 < 0.0
                or self.market_overnight_gap_q10_v1 > self.market_overnight_gap_q90_v1
                or self.market_total_transition_q10_v1 > self.market_total_transition_q90_v1
            ):
                raise ValueError("opening calibration thresholds are invalid")
            if self.calibration_missing_reason_v1 is not None:
                raise ValueError("complete opening calibration cannot have a missing reason")
...
class OpeningTransitionThresholdManifestV1(BaseModel):
    ...
    stock_relative_response_quintile_boundaries_v1: tuple[float, float, float, float]
...
def calculate_opening_preentry_window_v1(
    *,
    ...
    reasons: list[str] = []
    expected_signal = session_open_timestamp + FIVE_MINUTES_V1 * EXPECTED_OPENING_BAR_COUNT_V1
    ...
    prior_close = (
        None if prior_regular_session_close is None else float(prior_regular_session_close)
    )
    ...
    if complete:
        ...
        opening_range = math.log(max(bar.high for bar in ordered) / min(bar.low for bar in ordered))
...
def classify_opening_market_transition_v1(
    *,
    ...
    if thresholds is None:
        ...
        reasons.append(
            thresholds.calibration_missing_reason_v1 or "opening_calibration_thresholds_incomplete"
        )
    values = (
    ...
    negative = (
        window.market_opening_return_v1 <= thresholds.market_opening_return_q10_v1
        and window.market_opening_range_v1 >= thresholds.market_opening_range_q75_v1
    )
    positive = (
        window.market_opening_return_v1 >= thresholds.market_opening_return_q90_v1
        and window.market_opening_range_v1 >= thresholds.market_opening_range_q75_v1
    )
    ...
    if negative:
        ...
        sign = 1
    elif window.market_opening_range_v1 >= thresholds.market_opening_range_q75_v1:
...
def calculate_stock_opening_response_v1(
    *,
    ...
    stock_range = math.log(max(bar.high for bar in ordered) / min(bar.low for bar in ordered))
    ...
    market_return = None if market_opening_return_v1 is None else float(market_opening_return_v1)
    ...
    if relative > 0.0:
        ...
        subtype = "RESISTING_BUT_STILL_ALIGNED" if alignment > 0.0 else "ABSOLUTELY_OPPOSING"
    else:
