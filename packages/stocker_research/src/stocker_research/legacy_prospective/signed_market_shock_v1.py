...
def calculate_preentry_windows_v1(
    *,
    ...
    relevant = [
        ...
        if bar.symbol == market_proxy and bar.session == session and bar.bar_ordinal in required
    ]
    by_ordinal: dict[int, MarketShockBarV1] = {}
    reasons: list[str] = []
    for bar in relevant:
    ...
    for previous, current in zip(ordered, ordered[1:], strict=False):
        if (
            ...
            reasons.append(
                f"non_contiguous_market_timestamps:{previous.bar_ordinal}-{current.bar_ordinal}"
            )
    ...
    if value > FROZEN_SHOCK_CHECKPOINTS_V1[0] and not invalid_ordinals.intersection(w1_required):
    ...
    maximum_timestamp = (
        max((bar.bar_complete_timestamp for bar in ordered), default=None) if ordered else None
    )
...
def classify_market_shock_state_v1(
    *,
    ...
    if thresholds is None:
        ...
        reasons.append(
            thresholds.calibration_missing_reason_v1 or "shock_calibration_thresholds_incomplete"
        )
    required_values = (
    ...
    *,
    ...
    for previous, current in zip(ordered, ordered[1:], strict=False):
        if current.bar_start_timestamp != previous.bar_complete_timestamp:
            reasons.append(
                f"non_contiguous_stock_timestamps:{previous.bar_ordinal}-{current.bar_ordinal}"
            )
    ...
    *,
    ...
    if market_shock_state_v1.market_shock_state_v1 not in {
        ...
        if market_shock_state_v1.market_shock_state_v1 == "UNKNOWN_INCOMPLETE":
            reasons = market_shock_state_v1.missing_reasons_v1 or ("market_shock_state_incomplete",)
            return StockShockResponseResultV1(
    ...
    if relative > 0.0:
        ...
        subtype = (
            "RESISTING_BUT_STILL_WITH_SHOCK" if alignment > 0.0 else "ABSOLUTELY_OPPOSING_SHOCK"
        )
    else:
