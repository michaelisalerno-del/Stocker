...
def _provider_record(
    ...
    *,
    contract_id: str = "AAL250117C00100000",
    expiration_date: str = "2025-01-17",
) -> dict[str, object]:
    return {
        "id": f"{contract_id}-{observation_date}",
        "type": "options-eod",
        "attributes": {
            ...
            "open_interest": 100,
            "volatility": 0.40,
            "delta": 0.50 if "C" in contract_id[-9:] else -0.50,
            "bid_date": f"{observation_date} 15:59:58-05:00",
            "ask_date": f"{observation_date} 15:59:59-05:00",
            "tradetime": f"{observation_date} 15:30:00-05:00",
        },
    }


def test_exact_date_filter_retains_response_containing_only_requested_date() -> None:
    record = _provider_record("2025-01-02")

    result = filter_exact_observation_date(
        [record],
        requested_observation_date=date(2025, 1, 2),
        response_hash="response-hash",
        pagination_complete=True,
    )

    assert result.status == "exact_date_complete"
    assert result.retained_records == (record,)
    assert result.returned_observation_dates == (date(2025, 1, 2),)
    assert result.exact_date_record_count == 1


def test_exact_date_filter_discards_extra_provider_dates_before_materialisation() -> None:
    requested = _provider_record("2024-10-08")
    future = _provider_record("2024-10-10", contract_id="AAL250117P00100000")

    result = filter_exact_observation_date(
        [requested, future],
        requested_observation_date=date(2024, 10, 8),
        response_hash="mixed-response",
        pagination_complete=True,
    )

    assert result.status == "extra_dates_discarded"
    assert result.retained_records == (requested,)
    assert result.returned_observation_dates == (
        date(2024, 10, 8),
        date(2024, 10, 10),
    )
    assert result.discarded_other_date_record_count == 1


def test_exact_date_filter_discards_extra_before_validating_its_stale_quote_dates() -> None:
    requested = _provider_record("2024-11-21")
    stale_extra = _provider_record("2024-11-27", contract_id="AAL241206C00010500")
    attributes = stale_extra["attributes"]
    assert isinstance(attributes, dict)
    attributes["bid_date"] = "2024-11-26 15:59:58-05:00"
    attributes["ask_date"] = "2024-11-26 15:59:59-05:00"
    attributes["tradetime"] = "2024-11-21"

    result = filter_exact_observation_date(
        [requested, stale_extra],
        requested_observation_date=date(2024, 11, 21),
        response_hash="response-with-stale-extra",
        pagination_complete=True,
    )
...
def test_exact_date_filter_marks_response_missing_requested_date() -> None:
    result = filter_exact_observation_date(
        [_provider_record("2025-01-03")],
        requested_observation_date=date(2025, 1, 2),
        response_hash="wrong-date-response",
        pagination_complete=True,
    )

    assert result.status == "exact_date_absent"
    assert result.requested_date_present is False
    assert result.retained_records == ()
    assert result.exact_date_record_count == 0


def test_exact_date_filter_maps_unquoted_row_by_provider_identity() -> None:
    quoted = _provider_record("2024-05-01")
    unquoted = _provider_record("2024-05-01", contract_id="AAL240503P00010000")
    attributes = unquoted["attributes"]
    assert isinstance(attributes, dict)
    attributes["bid"] = None
    attributes["ask"] = 0.05
    attributes["bid_date"] = None
    attributes["ask_date"] = None

    result = filter_exact_observation_date(
        [quoted, unquoted],
        requested_observation_date=date(2024, 5, 1),
        response_hash="response-with-unquoted-row",
        pagination_complete=True,
    )

    assert result.status == "exact_date_complete"
    assert result.retained_records == (quoted, unquoted)
    assert result.exact_date_record_count == 2


def test_exact_date_filter_discards_post_boundary_extra_before_materialisation() -> None:
    requested = _provider_record(
        "2025-08-22",
        contract_id="AAL250919C00010000",
        expiration_date="2025-09-19",
    )
    protected_extra = _provider_record(
        "2025-08-25",
        contract_id="AAL250919P00010000",
        expiration_date="2025-09-19",
    )

    result = filter_exact_observation_date(
        [requested, protected_extra],
        requested_observation_date=date(2025, 8, 22),
        response_hash="response-with-protected-extra",
        pagination_complete=True,
    )
    ...
    assert result.retained_records == (requested,)
    assert result.discarded_other_date_record_count == 1
    assert result.discarded_post_boundary_record_count == 1
    assert result.exact_date_hash
    assert result.discarded_fragment_hash


def test_exact_date_filter_fails_closed_for_incomplete_pagination() -> None:
    result = filter_exact_observation_date(
        [_provider_record("2025-01-02")],
        requested_observation_date=date(2025, 1, 2),
        response_hash="incomplete-response",
        pagination_complete=False,
    )

    assert result.status == "incomplete_pagination"
    assert result.retained_records == ()
    assert result.requested_date_present is True
    assert result.exact_date_record_count == 1
    assert result.pagination_complete is False


def test_exact_date_filter_rejects_ambiguous_provider_date_mapping() -> None:
    record = _provider_record("2025-01-02")
    attributes = record["attributes"]
    assert isinstance(attributes, dict)
    attributes["ask_date"] = "2025-01-03 15:59:59-05:00"

    result = filter_exact_observation_date(
        [record],
        requested_observation_date=date(2025, 1, 2),
        response_hash="ambiguous-response",
        pagination_complete=True,
    )

    assert result.status == "ambiguous_date_mapping"
    assert result.retained_records == ()


def test_exact_date_filter_rejects_unverifiable_provider_schema() -> None:
    record = _provider_record("2025-01-02")
    del record["id"]
    ...
    result = filter_exact_observation_date(
        [record],
        requested_observation_date=date(2025, 1, 2),
        response_hash="schema-failure-response",
        pagination_complete=True,
    )
...
def test_expiration_beyond_boundary_is_allowed_for_pre_boundary_quote() -> None:
    validate_observation_and_expiration_boundary(
        observation_date=date(2025, 8, 22),
        expiration_date=date(2025, 9, 19),
    )


def test_exit_observation_beyond_boundary_is_rejected() -> None:
    with pytest.raises(ValueError, match="protected observation boundary"):
        validate_observation_and_expiration_boundary(
            observation_date=date(2025, 8, 25),
            expiration_date=date(2025, 9, 19),
        )


def test_v01_safety_flags_are_complete_and_strict() -> None:
    assert_safety_flags_v01(SAFETY_FLAGS_V01)
    assert SAFETY_FLAGS_V01["stock_signal_time"] == "15:30 America/New_York"
    assert SAFETY_FLAGS_V01["option_entry_time_proxy"] == "same_session_end_of_day_quote"
    assert (
        SAFETY_FLAGS_V01["option_exit_time_proxy"] == "future_end_of_day_quote_or_expiry_intrinsic"
    )
    unsafe = dict(SAFETY_FLAGS_V01)
    unsafe["extra_provider_dates_discarded_before_materialisation"] = False
    with pytest.raises(ValueError, match="safety flag"):
        assert_safety_flags_v01(unsafe)


def test_s2_direction_blocker_does_not_propagate_to_s1_or_s3() -> None:
    statuses = {
        "S1": "supported",
        "S2_ALL": "blocked_direction_mapping_unavailable",
        "S2_VETO": "blocked_direction_mapping_unavailable",
        "S3": "not_supported",
    }
    ...
    assert (
        choose_overall_decision_v01(
            statuses=statuses,
            strategy_positive={"S1": True, "S2": False, "S3": False},
...
def test_bid_ask_entry_exit_and_primary_commission() -> None:
    ...
    assert sensitivity["commissions"] == 4.0
    assert sensitivity["net_pnl"] == -4.0
    assert sensitivity["total_initial_cash_debit"] == 502.0
...
def _load_experiment_runner() -> ModuleType:
    ...
    specification = importlib.util.spec_from_file_location("test_fixed_options_runner_v01", path)
...
def test_cached_response_recovery_discards_extra_date_and_prevents_redownload(
    tmp_path: Path,
) -> None:
    runner = _load_experiment_runner()
    required = pd.DataFrame(
        [
            {
                "symbol": "AAL",
                "option_date": "2024-10-08",
                "strategy": "S1",
                "role": "contract_preselection",
                "signal_session": "2024-10-09",
            }
        ]
    )
    ...
    plan = runner.plan_option_requests(required, signals)[0]
    run_dir = tmp_path / runner.V0_OPTIONS_RUN_DIR_NAME
    raw_dir = run_dir / "raw"
    manifest_dir = run_dir / "manifests" / "completed"
    raw_dir.mkdir(parents=True)
    manifest_dir.mkdir(parents=True)
    requested_record = _provider_record(
        "2024-10-08",
        contract_id="AAL241018C00010000",
        expiration_date="2024-10-18",
    )
    requested_attributes = requested_record["attributes"]
    assert isinstance(requested_attributes, dict)
    requested_attributes["dte"] = 9
    unquoted_record = _provider_record(
        "2024-10-08",
        contract_id="AAL241018P00010000",
        expiration_date="2024-10-18",
    )
    unquoted_attributes = unquoted_record["attributes"]
    assert isinstance(unquoted_attributes, dict)
    unquoted_attributes["bid"] = None
    unquoted_attributes["ask"] = None
    unquoted_attributes["bid_date"] = None
    unquoted_attributes["ask_date"] = None
    payload = {
        "data": [
            requested_record,
            unquoted_record,
        ...
        "meta": {"offset": 0, "limit": 1000},
        "links": {"next": None},
    }
    raw_content = json.dumps(payload, sort_keys=True).encode()
    response_hash = hashlib.sha256(raw_content).hexdigest()
    raw_path = raw_dir / f"{response_hash}.json"
    raw_path.write_bytes(raw_content)
    (manifest_dir / f"{plan.resume_id}.json").write_text(
        json.dumps(
            {
                "manifest_rows": [
                    {
    ...
    (run_dir / "bounded_download_manifest.json").write_text(
        json.dumps(
            {
                "queries": [
                    {
    ...
    canonical, audit, _receipts, summary = runner.reprocess_cached_responses(
        required=required,
        signals=signals,
        options_cache=tmp_path,
    )

    assert canonical["trade_date"].tolist() == [date(2024, 10, 8)]
    assert audit["status"].tolist() == ["extra_dates_discarded"]
    assert audit["discarded_other_date_record_count"].tolist() == [1]
    assert summary["previously_missing_chains_recovered"] == 1
    assert summary["provider_dte_disagreement_records"] == 1
    assert summary["unquotable_exact_date_records_discarded"] == 1
    assert runner.unresolved_option_requests(required, signals, audit) == []
...
def test_unexpected_contract_selection_error_fails_closed(
    ...
    signals = pd.DataFrame(
        [
            {
                "row_id": "AAL|2024-01-04|72",
                "symbol": "AAL",
                "session": "2024-01-04",
                "period": "development",
                "contract_selection_date": "2024-01-03",
                "exit_session": "2024-01-05",
                "ordinal_72_structural_available": True,
                "chronology_eligible": True,
                "underlying_source_available": True,
                "split_boundary_ambiguous": False,
    ...
    options = pd.DataFrame(
        [
            {
                "underlying_symbol": "AAL",
                "contract_id": "AAL240112C00010000",
    ...
    def unexpected_failure(*_args: object, **_kwargs: object) -> None:
        raise RuntimeError("unexpected selection regression")

    monkeypatch.setattr(runner, "select_atm_straddle", unexpected_failure)
    with pytest.raises(RuntimeError, match="unexpected selection regression"):
        runner.construct_straddle_economics(signals, options)
