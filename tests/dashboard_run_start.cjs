...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator("details").evaluate(node => node.open), true,
      "Starting with missing required risk settings must reveal the fields");
    assert.equal(await page.locator("[name=max_gross_notional]").evaluate(node => node === document.activeElement), true);
    assert.equal(posts, 0, "Invalid risk settings must not start a run");
    await page.locator("[name=max_gross_notional]").fill("100000");
    await page.locator("[name=risk_per_trade]").fill("0");
    await page.locator("details summary").click();
    await page.getByRole("button", { name: "Start PAPER run" }).click();
    assert.equal(await page.locator("details").evaluate(node => node.open), true);
    assert.equal(posts, 0, "Out-of-range risk must not start a run");
    await page.locator("[name=risk_per_trade]").fill("0.1");
    await page.getByRole("button", { name: "Start PAPER run" }).click();
