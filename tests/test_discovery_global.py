    ...
    async def scenario():
        ...
        async def quote(instrument):
            return CurrentQuote(
                "GBP",
                456,
                now - timedelta(minutes=5) if fault == "stale" else now,
                None if fault == "missing" else 1.3,
                1.2 if fault == "crossed" else 1.4,
                None,
                None,
                3 if fault == "delayed" else 1,
            )
