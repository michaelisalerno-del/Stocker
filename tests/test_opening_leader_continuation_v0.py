...
def test_deployment_freeze_receipt_binds_artifacts_sources_and_boundary(
    ...
    shutil.copytree(
        ...
        ignore=shutil.ignore_patterns(
            ...
            "deployment_freeze_receipt_v14.json",
            "deployment_freeze_receipt_v15.json",
            "deployment_freeze_receipt_v16.json",
            "deployment_freeze_receipt_v17.json",
            "deployment_freeze_receipt_v18.json",
            "deployment_freeze_receipt_v19.json",
            ...
            "deployment_freeze_receipt_v26.json",
            "deployment_freeze_receipt_v27.json",
            "deployment_freeze_receipt_v28.json",
            "deployment_freeze_receipt_v29.json",
            "deployment_freeze_receipt_v30.json",
            "deployment_freeze_receipt_v31.json",
        ),
    )
...
def test_committed_opening_leader_refreeze_preserves_original_and_binds_current_sources() -> None:
    ...
    assert isinstance(receipt, OpeningLeaderDeploymentRefreezeReceiptV31)
    assert receipt.recorder_version == "opening-leader-continuation-recorder-v0"
    assert (
        receipt.supersedes_receipt_sha256
        == hashlib.sha256((package / "deployment_freeze_receipt_v30.json").read_bytes()).hexdigest()
    )
    previous = json.loads(
        (package / "deployment_freeze_receipt_v30.json").read_text(encoding="utf-8")
    )
    assert receipt.supersedes_deployment_receipt_id == previous["deployment_receipt_id"]
    assert receipt.refreeze_reason == "operational_bounded_batch_partition_hash_storage"
...
def test_opening_leader_v3_refreeze_cannot_move_freeze_boundary_backward(
    tmp_path: Path,
) -> None:
    source_package = (
        Path(__file__).parents[1]
    ...
    package = tmp_path / "opening-leader-package"
    shutil.copytree(source_package, package)
    prior = OpeningLeaderDeploymentRefreezeReceiptV2.model_validate_json(
        (package / "deployment_freeze_receipt_v2.json").read_text(encoding="utf-8")
    )
    refreeze_path = package / "deployment_freeze_receipt_v3.json"
    payload = json.loads(refreeze_path.read_text(encoding="utf-8"))
    payload["freeze_completed_at_utc"] = (
        prior.freeze_completed_at_utc - timedelta(seconds=1)
    ).isoformat()
    payload["deployment_receipt_id"] = "olc-deploy-placeholder"
    payload["signature_sha256"] = "0" * 64
    provisional = OpeningLeaderDeploymentRefreezeReceiptV3.model_validate(payload)
    with_id = provisional.model_copy(
        update={
            "deployment_receipt_id": opening_leader_live._expected_deployment_receipt_id(
                provisional
    ...
    with pytest.raises(ValueError, match="V3 deployment refreeze lineage mismatch"):
        load_opening_leader_package_v0(
            package,
            source_files=opening_leader_runtime_source_files_v0(),
        )
...
def test_opening_leader_v4_refreeze_cannot_move_freeze_boundary_backward(
    tmp_path: Path,
) -> None:
    source_package = (
        Path(__file__).parents[1]
    ...
    package = tmp_path / "opening-leader-package"
    shutil.copytree(source_package, package)
    prior = OpeningLeaderDeploymentRefreezeReceiptV3.model_validate_json(
        (package / "deployment_freeze_receipt_v3.json").read_text(encoding="utf-8")
    )
    refreeze_path = package / "deployment_freeze_receipt_v4.json"
    payload = json.loads(refreeze_path.read_text(encoding="utf-8"))
    payload["freeze_completed_at_utc"] = (
        prior.freeze_completed_at_utc - timedelta(seconds=1)
    ).isoformat()
    payload["deployment_receipt_id"] = "olc-deploy-placeholder"
    payload["signature_sha256"] = "0" * 64
    provisional = OpeningLeaderDeploymentRefreezeReceiptV4.model_validate(payload)
    with_id = provisional.model_copy(
        update={
            "deployment_receipt_id": opening_leader_live._expected_deployment_receipt_id(
                provisional
    ...
    with pytest.raises(ValueError, match="V4 deployment refreeze lineage mismatch"):
        load_opening_leader_package_v0(
            package,
            source_files=opening_leader_runtime_source_files_v0(),
        )
