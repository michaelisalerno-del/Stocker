...
def test_committed_opening_leader_refreeze_preserves_original_and_binds_current_sources() -> None:
    ...
    assert isinstance(receipt, OpeningLeaderDeploymentRefreezeReceiptV4)
    assert receipt.recorder_version == "opening-leader-continuation-recorder-v0"
    assert receipt.supersedes_receipt_sha256 == (
        "5849ca7bfeb3cb55bd8f941cfef43a43616ebd14c9c3ff026991b7c72f7f72b5"
    )
    assert receipt.supersedes_deployment_receipt_id == ("olc-deploy-82243a6a8028deec07c8a968")
    assert receipt.refreeze_reason == "record_only_option_risk_accounting_source_update"
