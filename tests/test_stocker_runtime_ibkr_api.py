...
import stocker_runtime.ingestion.ibkr_api as ibkr_module
...
def test_unverified_ibapi_tree_is_rejected_before_package_code_executes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    package = tmp_path / "ibapi"
    ...
    monkeypatch.delitem(sys.modules, "ibapi", raising=False)
    provenance = tmp_path / "provenance.json"
    provenance.write_text(
        OfficialIBKRApiProvenance(
            schema_version="1",
