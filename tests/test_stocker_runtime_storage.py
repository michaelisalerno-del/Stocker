...
def test_market_open_reliability_migration_preserves_deployed_v2_evidence(
    ...
    assert result.applied_versions == (16, 17, 18, 19, 20)
    with connect_v2(database) as connection:
...
def test_payload_compaction_index_migration_preserves_schema_16_evidence(
    ...
    with connect_v2(database, verify_schema=False) as connection:
        ...
        plans = {
            ...
            "acknowledged_compaction": " ".join(
                str(row["detail"])
                for row in connection.execute(
                    f"EXPLAIN QUERY PLAN {ACK_PAYLOAD_COMPACTION_SQL}",
                    ("retention-run", 10, 10, 2_000),
                )
            ),
            "failed_compaction": " ".join(
                str(row["detail"])
                for row in connection.execute(
                    f"EXPLAIN QUERY PLAN {FAILED_PAYLOAD_COMPACTION_SQL}",
                    ("retention-run", 10, 10, 2_000),
                )
            ),
...
def test_component_incident_generation_migration_preserves_schema_17_evidence(
    tmp_path: Path,
) -> None:
    database = tmp_path / "schema-17.sqlite3"
    migration_root = tmp_path / "schema-17-migrations"
    migration_root.mkdir()
    for migration in migration_plan()[:17]:
        (migration_root / migration.name).write_text(migration.sql, encoding="utf-8")
    initialize_database(database, migration_root=migration_root, applied_at_us=1)
    with connect_v2(database, verify_schema=False) as connection:
        connection.execute(
            "INSERT INTO runs(run_id, mode, source, started_at_us, config_hash, git_commit, "
            "data_class, status) VALUES ('incident-run', 'shadow', 'ibkr', 10, ?, "
            "'old-commit', 'shadow_protected', 'running')",
            ("a" * 64,),
        )
        connection.execute(
            "INSERT INTO incidents(incident_id, run_id, scope, severity, code, opened_at_us, "
            "details_json) VALUES ('old-component', 'incident-run', 'component', "
            "'degraded', 'COMPONENT_RETENTION_MAINTENANCE_FAILED', 11, '{}')"
        )
        before = tuple(connection.execute("SELECT * FROM incidents"))[0]
    ...
    with connect_v2(database) as connection:
        after = connection.execute(
            ...
            "recorder_generation FROM incidents"
        ).fetchone()
        column = connection.execute("PRAGMA table_info(incidents)").fetchall()[-1]
        foreign_keys = tuple(connection.execute("PRAGMA foreign_key_check"))
        quick_check = connection.execute("PRAGMA quick_check").fetchone()[0]
        with pytest.raises(sqlite3.IntegrityError, match="CHECK constraint failed"):
            connection.execute(
                "INSERT INTO incidents(incident_id, run_id, scope, severity, code, "
                "opened_at_us, details_json, recorder_generation) VALUES "
                "('invalid-generation', 'incident-run', 'component', 'degraded', "
                "'TEST', 12, '{}', -1)"
            )
...
def test_readiness_latest_index_migration_preserves_schema_18_evidence(
    ...
    migration_19 = migration_plan()[18]
    (migration_root / migration_19.name).write_text(migration_19.sql, encoding="utf-8")
    result = migrate_database(database, applied_at_us=2, migration_root=migration_root)
...
def test_admission_freshness_migration_preserves_schema_19_evidence(
    tmp_path: Path,
) -> None:
    database = tmp_path / "schema-19.sqlite3"
    migration_root = tmp_path / "schema-19-migrations"
    migration_root.mkdir()
    for migration in migration_plan()[:19]:
        (migration_root / migration.name).write_text(migration.sql, encoding="utf-8")
    initialize_database(database, migration_root=migration_root, applied_at_us=1)
    with connect_v2(database, verify_schema=False) as connection:
        connection.execute(
            "INSERT INTO runs(run_id, mode, source, started_at_us, config_hash, git_commit, "
            "data_class, status) VALUES ('freshness-run', 'prospective_record', 'ibkr', 1, ?, "
            "'old-commit', 'prospective_protected', 'running')",
            ("a" * 64,),
        )
        connection.execute(
            "INSERT INTO recorder_generations(run_id, generation, owner_id, started_at_us) "
            "VALUES ('freshness-run', 1, 'old-owner', 1)"
        )
        connection.execute(
            "INSERT INTO instruments(instrument_id, identity_hash, kind, symbol, exchange, "
            "currency) VALUES ('instrument-1', ?, 'stock', 'XYZ', 'SMART', 'USD')",
        ...
        connection.execute(
            "INSERT INTO subscriptions(subscription_id, run_id, recorder_generation, "
            "connection_generation, instrument_id, feed_kind, request_id, lifecycle, "
            "requirements_hash, opened_at_us, stale_after_us) VALUES "
            "('subscription-1', 'freshness-run', 1, 1, 'instrument-1', 'quotes', 7, "
            "'active', ?, 10, 15000000)",
            ("c" * 64,),
        )
        connection.execute(
            ...
            "(1, 'pending-callback', 'freshness-run', 1, 1, 7, 'quote', 11, ?, 'pending')",
            ("d" * 64,),
        )
        subscription_before = tuple(connection.execute("SELECT * FROM subscriptions"))[0]
        callback_before = tuple(connection.execute("SELECT * FROM callback_inbox"))[0]

    migration_20 = migration_plan()[19]
    (migration_root / migration_20.name).write_text(migration_20.sql, encoding="utf-8")
    result = migrate_database(database, applied_at_us=2, migration_root=migration_root)

    assert result.applied_versions == (20,)
    with connect_v2(database) as connection:
        subscription_after = connection.execute("SELECT * FROM subscriptions").fetchone()
        callback_after = tuple(connection.execute("SELECT * FROM callback_inbox"))[0]
        obsolete_index = connection.execute(
        ...
        with pytest.raises(sqlite3.IntegrityError, match="CHECK constraint failed"):
            connection.execute(
                "UPDATE subscriptions SET last_admitted_callback_at_us=9 "
                "WHERE subscription_id='subscription-1'"
            )
    assert tuple(subscription_after[:-1]) == tuple(subscription_before)
    assert subscription_after[-1] is None
    assert callback_after == callback_before
    assert obsolete_index is None
    assert foreign_keys == ()
    assert quick_check == "ok"
...
def test_callback_receipt_frontier_index_migration_preserves_rows_and_is_used(
    ...
    database = tmp_path / "receipt-frontier.sqlite3"
    migration_root = tmp_path / "schema-14-migrations"
    migration_root.mkdir()
    for migration in migration_plan()[:14]:
        (migration_root / migration.name).write_text(migration.sql, encoding="utf-8")
    initialize_database(database, migration_root=migration_root, applied_at_us=1)
    with connect_v2(database, verify_schema=False) as connection:
        connection.execute(
        ...
        connection.executemany(
            ...
            "payload_json,payload_sha256,lifecycle,receipt_batch_id) VALUES "
            "(?,?,'run-1',1,1,'quote',?,'{}',?,'failed','prior-receipt')",
            (
                (sequence, f"receipted-{sequence}", sequence, f"{sequence:064x}")
                for sequence in range(1, 1_001)
            ),
        )
        for sequence, lifecycle in enumerate(("failed", "pending"), start=1_001):
            connection.execute(
                "INSERT INTO callback_inbox(source_sequence,event_uid,run_id,"
                "recorder_generation,connection_generation,callback_kind,received_at_us,"
                "payload_json,payload_sha256,lifecycle) VALUES "
                "(?,?,'run-1',1,1,'quote',?,'{}',?,?)",
                (sequence, f"event-{sequence}", sequence, f"{sequence:064x}", lifecycle),
            )
        before = tuple(connection.execute("SELECT * FROM callback_inbox ORDER BY source_sequence"))
    ...
    with connect_v2(database) as connection:
        ...
        index_sql = connection.execute(
            ...
            "WHERE type='index' AND name='callback_inbox_unreceipted_terminal_idx'"
        ).fetchone()[0]
        plan = tuple(
            str(row[3])
            for row in connection.execute(
                "EXPLAIN QUERY PLAN SELECT run_id FROM callback_inbox "
        ...
        quick_check = connection.execute("PRAGMA quick_check").fetchone()[0]
    assert after == before
    assert "WHERE receipt_batch_id IS NULL" in index_sql
    assert any("callback_inbox_unreceipted_terminal_idx" in step for step in plan), plan
    assert foreign_keys == ()
    assert quick_check == "ok"
...
def test_phase2_migration_preserves_dynamic_rows_and_admits_only_causal_derived_receipts(
    ...
    assert result.applied_versions == (13, 14, 15, 16, 17, 18, 19, 20)
    with connect_v2(database) as connection:
        assert (
            connection.execute(
...
def test_retention_compacts_full_batch_with_one_set_based_update_under_loaded_clock(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    database = tmp_path / "v2.sqlite3"
    ...
    with connect_v2(database) as connection:
        connection.executemany(
            ...
            "(?, 'retention-run', 1, 1, 'tick', 1, '{\"private\":\"evidence\"}', ?, "
            "'pending', 'bulk-receipt')",
            ((f"bulk-{index}", "a" * 64) for index in range(batch_rows)),
        )
        connection.execute(
            "INSERT INTO market_events(event_id, run_id, source_sequence, instrument_id, "
            "feed_kind, event_kind, event_at_us, received_at_us, connection_generation, "
            "payload_json, payload_sha256) SELECT 'bulk-event-' || source_sequence, run_id, "
            "source_sequence, 'instrument-1', 'trades', 'tick', received_at_us, "
            "received_at_us, connection_generation, '{}', ? FROM callback_inbox "
            "WHERE run_id='retention-run'",
            ("b" * 64,),
        )
        connection.execute(
            "UPDATE callback_inbox SET lifecycle='acknowledged', "
            "normalized_event_id='bulk-event-' || source_sequence, acknowledged_at_us=1 "
            "WHERE run_id='retention-run'"
        )
        last_sequence = int(
            connection.execute(
        ...
        connection.execute(
            ...
            "last_receipt_chain_hash, updated_at_us) VALUES "
            "('retention-run', ?, ?, 1, 1, ?, ?, 1)",
            (last_sequence, batch_rows, "0" * 64, "0" * 64),
        )

    logical_time = 0.0
    payload_update_statements: set[str] = set()
    ...
    def traced_connect(path: str | Path) -> sqlite3.Connection:
        ...
        def trace(statement: str) -> None:
            ...
            if (
                normalized.startswith("UPDATE CALLBACK_INBOX SET PAYLOAD_JSON = NULL")
                and normalized not in payload_update_statements
            ):
                logical_time += 0.000_06
                payload_update_statements.add(normalized)
    ...
    result = RetentionManager(
        ...
        RetentionPolicy(
            ...
            receipt_us=1_000,
            tombstone_us=1_000,
            maintenance_batch_rows=batch_rows,
            maintenance_transaction_ms=100,
        ),
        monotonic=lambda: logical_time,
    ).run(now_us=100, measured_database_bytes=1, measured_wal_bytes=0)

    assert result.payloads_compacted == batch_rows
    assert len(payload_update_statements) == 1
    with connect_v2(database) as connection:
        assert (
            connection.execute(
                "SELECT count(*) FROM callback_inbox WHERE run_id='retention-run'"
            ).fetchone()[0]
            == batch_rows
        )
        assert (
        ...
        assert (
            connection.execute(
                "SELECT count(*) FROM market_events WHERE run_id='retention-run'"
            ).fetchone()[0]
            == batch_rows
        )
...
@pytest.mark.parametrize(
    ("batch_rows", "expected_statements", "expected_null_payloads"),
    ((2, 1, 2), (3, 2, 3)),
)
def test_retention_set_based_updates_share_acknowledged_first_failed_budget(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    batch_rows: int,
    expected_statements: int,
    expected_null_payloads: int,
) -> None:
    database = tmp_path / "v2.sqlite3"
    initialize_database(database)
    _seed_output_dependencies(database)
    acknowledged = (
        _seed_callback_for_retention(database, uid="ack-1", received_at_us=1),
        ...
        failed = tuple(
            int(
                connection.execute(
                    "INSERT INTO callback_inbox(event_uid, run_id, recorder_generation, "
                    "connection_generation, callback_kind, received_at_us, payload_json, "
                    "payload_sha256, lifecycle, failure_code, receipt_batch_id) VALUES "
                    "(?, 'retention-run', 1, 1, 'tick', ?, '{}', ?, 'failed', "
                    "'fixture-failure', 'failed-receipt')",
                    (f"failed-{index}", index + 3, "c" * 64),
                ).lastrowid
            )
    ...
    def traced_connect(path: str | Path) -> sqlite3.Connection:
        ...
        def trace(statement: str) -> None:
            normalized = " ".join(statement.upper().split())
            if normalized.startswith("UPDATE CALLBACK_INBOX SET PAYLOAD_JSON = NULL"):
                payload_update_statements.add(normalized)
    ...
    assert result.payloads_compacted == batch_rows
    assert len(payload_update_statements) == expected_statements
    with connect_v2(database) as connection:
        payloads = {
            int(row["source_sequence"]): row["payload_json"]
            for row in connection.execute(
                "SELECT source_sequence, payload_json FROM callback_inbox "
                "WHERE run_id='retention-run' ORDER BY source_sequence"
            )
        }
    assert sum(payload is None for payload in payloads.values()) == expected_null_payloads
...
def test_retention_deadline_during_failed_set_update_rolls_back_acknowledged_update(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    database = tmp_path / "v2.sqlite3"
    initialize_database(database)
    _seed_output_dependencies(database)
    acknowledged = _seed_callback_for_retention(
        database,
        uid="set-update-acknowledged",
        received_at_us=1,
        receipt_batch_id="set-update-receipt",
    )
    with connect_v2(database) as connection:
        failed = int(
            connection.execute(
                ...
                "connection_generation, callback_kind, received_at_us, payload_json, "
                "payload_sha256, lifecycle, failure_code, receipt_batch_id) VALUES "
                "('set-update-failed', 'retention-run', 1, 1, 'tick', 2, '{}', ?, "
                "'failed', 'fixture-failure', 'set-update-receipt')",
                ("c" * 64,),
            ).lastrowid
        )
        connection.execute("UPDATE runs SET status='stopped' WHERE run_id='retention-run'")
    ...
    def traced_connect(path: str | Path) -> sqlite3.Connection:
        ...
        def trace(statement: str) -> None:
            nonlocal logical_time
            normalized = " ".join(statement.upper().split())
            if normalized.startswith("UPDATE CALLBACK_INBOX SET PAYLOAD_JSON = NULL") and (
                "CANDIDATE.LIFECYCLE = 'FAILED'" in normalized
            ):
                logical_time = 1.0
    ...
    assert payloads == ('{"private":"evidence"}', "{}")
...
def test_database_cli_is_machine_readable_and_never_returns_payloads(tmp_path: Path) -> None:
    ...
    assert migration_payload == {"applied_versions": [], "current_version": 20, "status": "ok"}
