...
POLLING = (STATIC_ROOT / "polling.mjs").read_text(encoding="utf-8")
...
def test_fast_poll_only_requests_the_compact_summary() -> None:
    fast_body = APPLICATION.split("async function refreshFast", 1)[1].split(
        "async function refreshScreenTier",
        1,
    )[0]
...
def test_polling_supersedes_manual_refresh_and_pauses_while_hidden() -> None:
    ...
    assert "if (supersede || refreshDetails) this.controller.abort()" in POLLING
...
def test_heavy_data_is_routed_by_visible_screen_and_polling_loads_first() -> None:
    ...
    assert 'from "/assets/polling.mjs"' in APPLICATION
    assert '<script src="/assets/app.js" type="module"></script>' in INDEX
